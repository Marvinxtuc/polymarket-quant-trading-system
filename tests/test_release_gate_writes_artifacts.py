...
class ReleaseGateArtifactsTests(unittest.TestCase):
    def test_release_gate_writes_json_and_markdown_with_identity_metadata(self):
        ...
        with tempfile.TemporaryDirectory(prefix="release-gate-artifacts-") as temp_dir:
            root = Path(temp_dir)
            ...
            self.assertGreater(md_out.stat().st_size, 0)

            data = json.loads(json_out.read_text(encoding="utf-8"))
            self.assertEqual(data["go_no_go"], "GO")
            self.assertEqual(data["release_gate_command"], "bash scripts/gates/gate_release_readiness.sh")
            self.assertEqual(data["required_blocks"], required_blocks)
            self.assertIn("execution_timestamp_utc", data)
            self.assertIn("git_branch", data)
            self.assertIn("git_commit", data)
            self.assertIn("Decision: **GO**", md_out.read_text(encoding="utf-8"))
